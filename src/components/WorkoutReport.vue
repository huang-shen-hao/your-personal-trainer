    ...
    <div v-loading="loading">
      ...
      <el-row :gutter="20" class="stats-row">
          ...
          <el-card shadow="hover">
            <div class="stat-item">
              ...
              <div class="stat-value">
                {{ reportData?.stats.totalWorkouts || 0 }}
              </div>
              <div
                v-if="reportData?.comparison"
                class="stat-change"
                :class="getChangeClass(reportData.comparison.frequencyChange)"
              >
        ...
        <el-col :sm="8" :xs="24">
          <el-card shadow="hover">
            <div class="stat-item">
              <div class="stat-label">训练总时间</div>
              <div class="stat-value">
                {{ reportData?.stats.totalDuration || 0 }}
                <span class="unit">分钟</span>
              </div>
              <div
                v-if="reportData?.comparison"
                class="stat-change"
                :class="getChangeClass(reportData.comparison.durationChange)"
              >
        ...
        <el-col :sm="8" :xs="24">
          <el-card shadow="hover">
            <div class="stat-item">
              ...
              <div class="stat-value">
                {{ reportData?.stats.averageDuration || 0 }}
                <span class="unit">分钟</span>
              </div>
              <div
                v-if="reportData?.comparison"
      ...
      <el-card
        ...
        v-if="reportData && reportData.workouts.length > 0"
      >
        <template #header>
        ...
        <el-table :data="reportData.workouts" stripe style="width: 100%">
          ...
          <el-table-column label="动作数" min-width="100">
            <template #default="{ row }">
      ...
      <!-- 空状态 -->
      <el-empty
        v-if="!loading && (!reportData || reportData.workouts.length === 0)"
        description="暂无训练记录"
      />
    </div>
  </div>
...
async function loadReport() {
  ...
  try {
    if (reportType.value === "weekly") {
      reportData.value = await generateWeeklyReport(
        userStore.profile.id,
        currentDate.value
      );
    } else {
      reportData.value = await generateMonthlyReport(
        userStore.profile.id,
        currentDate.value
      );
    }
